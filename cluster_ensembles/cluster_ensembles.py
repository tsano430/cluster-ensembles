...
def mcla(base_clusters, nclass, random_state):
    ...
    meta_clusters = sparse.dok_matrix((base_clusters.shape[1], nclass), dtype=float).tolil()
...
def hbgf(base_clusters, nclass):
    ...
    rowA, colA = A.shape
    W = sparse.bmat([[sparse.dok_matrix((colA, colA)), A.T], [A, sparse.dok_matrix((rowA, rowA))]])
